...
public class ServiceAgent extends Agent {
	protected void setup () {
		...
		addBehaviour(new CustomCyclicBehaviour(this));
...
class CustomCyclicBehaviour extends CyclicBehaviour
{
	ServiceAgent agent;
	public CustomCyclicBehaviour(ServiceAgent agent)
	{
		this.agent = agent;
	}
	public void action()
	{
		MessageTemplate template = MessageTemplate.MatchOntology("pol-nor");
		ACLMessage message = agent.receive(template);
		if (message == null)
		{
			block();
		...
		else
		{
			...
			try
			{
				response = agent.makeRequest("fd-pol-nor",content);
			}
			catch (NumberFormatException ex)
			{
				response = ex.getMessage();
